...
2. **[节点 2: 图像生成节点 (Midjourney / SD IP-Adapter)]** → (输出 1张 16:9 包含 9个网格的分镜图 URL)
> ⚠️ **【电商避坑核心警告：拒绝虚假宣传！】**
> 纯粹的文本生图（如 DALL-E 3）会“凭空捏造”衣服的款式和印花，导致生成的商品与你实际售卖的商品不同，这是电商带货的大忌！
> **解决方案**：在节点2中，**绝对不能只输入文字**。你必须将用户上传的【原商品图】作为**垫图（Image Prompt）**输入给生图模型。
> - 如果使用 **Midjourney**，请在生图指令末尾加上垫图参数 `--cref <商品图URL> --cw 100` 或 `--sref <商品图URL>` 以 100% 锁死服装款式与印花。
> - 如果使用 **Stable Diffusion (ComfyUI)**，请务必串接 **IP-Adapter** 节点，并将商品图输入进去，实现完美换装和商品复刻。
...
### 第四部分：让静态图“动”起来 —— 爆款视频分镜脚本与 I2V 提示词生成

切出 9 张/16 张极品高清原图后，如何将它们交给 Luma / 可灵 / Runway 等视频生成 AI，并最终在剪映中组合成高转化率的大片？

你可以在你的智能体中加入以下**【后续节点提示词】**，让 AI 自动生成“剪辑台本”与对应的“英文 I2V (图生视频) 动效提示词”：

**【视频脚本与 I2V 提示词万能模版】**
```text
我刚才已经生成了一组【你的产品名称，如：MEJEND 黑帮T恤】的高清分镜切图。
为了把这些切好的静态图片串联成一个具备高转化率的带货视频，请为我进行两步包装：
第一步：生成“15秒爆款视频分镜故事板（表格）”；
第二步：为每个核心切片提供独立的高清视频生成（I2V）英文提示词。

请按以下格式严格输出：
一、 15秒爆款视频分镜脚本（流程表）
表格列必须包含：分镜编号 | 时长 | 画面视觉内容 | 镜头运动 | 旁白/台词 | 音效与BGM。
设计 5~6 个连贯的镜头，紧扣产品卖点，节奏紧凑，极具大片质感。

二、 分镜独立视频生成提示词（I2V 专属）
为每个核心镜头写一段英文 I2V 提示词，专为可灵 AI、Runway Gen-3、Luma 等视频模型优化。
要求包含：主体动作 (Subject Action)、镜头运动 (Camera Movement, 例如 Macro close-up, Slow zoom in)、光影环境 (Lighting & Atmosphere)。
格式必须为：
I2V Prompt: [纯英文提示词]
(中文大意：...)
```

把生成的 **I2V Prompt** 和刚才切好的**单张图片**组合喂给视频生成大模型，你就能得到具有超强一致性、且运镜丰富的高级电影感短视频素材！
最后，参照表格中的旁白与BGM，在剪映里一键组合即可完成商业交付！